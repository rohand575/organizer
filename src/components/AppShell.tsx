...
const tabs = [
  { to: '/todos', label: 'To-do', Icon: CheckCircleIcon },
      ...
      <aside className="sticky top-0 hidden h-screen w-56 shrink-0 flex-col py-8 md:flex">
        ...
        <div className="mt-auto flex items-center gap-3 px-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <button onClick={signOut} className="text-xs text-subtle hover:text-accent">
              Sign out
            </button>
          </div>
        </div>
        ...
        <header className="glass safe-top sticky top-0 z-20 flex items-center justify-between px-5 py-3 md:hidden">
          ...
          {user?.photoURL && (
            <button onClick={signOut}>
              <img src={user.photoURL} alt="Sign out" className="h-9 w-9 rounded-full" />
            </button>
          )}
        </header>
